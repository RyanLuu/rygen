...
struct iwrap {
  ...
  int flags;
  struct param params[MUSTACH_MAX_PARAMS];
  int nparams;
};
...
static int process(const char *template, size_t length, struct iwrap *iwrap, FILE *file,
  ...
  const char *op, *beg, *term, *end;
  struct {
    const char *name, *again;
    size_t length;
    unsigned type : 2, enabled : 1, entered : 1;
  } stack[MUSTACH_MAX_DEPTH];
  size_t oplen, cllen, len, l;
  int depth, rc, enabled, stdalone, iparam, nparams, type;
  struct prefix pref;
  ...
  for (;;) {
    ...
    case '/':
      ...
      case 0: /* end section */
        ...
        if (rc) {
          ...
          if (stack[depth].enabled && stack[depth].entered)
      ...
      case 1: /* end parent */
        ...
        if (rc < 0)
          return rc;
        iwrap->nparams = nparams;
        goto pop_stack;
      case 2: /* end block */
        if (stack[depth].type == 1) {
          iwrap->params[iwrap->nparams].arg_name = stack[depth].name;
          iwrap->params[iwrap->nparams].arg_len = stack[depth].length;
          iwrap->params[iwrap->nparams].value = stack[depth].again;
          iwrap->params[iwrap->nparams].len = op - stack[depth].again;
          iwrap->nparams++;
        } else {
          for (iparam = 0; iparam < iwrap->nparams; iparam++) {
            if (memcmp(iwrap->params[iparam].arg_name, name, len) == 0) {
              rc = process(iwrap->params[iparam].value, iwrap->params[iparam].len, iwrap, file,
                           &pref);
              if (rc < 0)
                return rc;
              break;
          ...
          if (iparam == iwrap->nparams) {
            rc = process(stack[depth].again, op - stack[depth].again, iwrap, file, &pref);
            if (rc < 0)
              return rc;
          }
        }
    ...
    case '<':
      /* parents */
      stack[depth].name = beg;
      stack[depth].again = NULL;
      stack[depth].length = len;
      stack[depth].type = type;
      type = 1;
      stack[depth].enabled = enabled != 0;
      stack[depth].entered = enabled;
      enabled = 0;
      depth++;
      nparams = iwrap->nparams;
      break;
    case '$':
      /* blocks */
      stack[depth].name = beg;
...
int mustach_file(const char *template, size_t length, const struct mustach_itf *itf, void *closure,
                 int flags, FILE *file) {
  int rc;
  struct iwrap iwrap;
...
int mustach_fd(const char *template, size_t length, const struct mustach_itf *itf, void *closure,
               int flags, int fd) {
  int rc;
  FILE *file;
...
int mustach_mem(const char *template, size_t length, const struct mustach_itf *itf, void *closure,
...
int mustach(const char *template, const struct mustach_itf *itf, void *closure, char **result,
            size_t *size) {
